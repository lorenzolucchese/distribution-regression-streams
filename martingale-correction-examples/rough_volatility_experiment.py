...
def run_experiment(N=20, M=50, length=200, min_T=1e-6, max_T=1e0, ll=None):
    ...
    for X, features in zip([P, V], ['price', 'vol']):
        ...
        SES_mse, _, results_SES = SES.model(X, y, ll=ll, at=True, NUM_TRIALS=5, cv=3, depths1=[2,3,4], depth2=2)
        SES_mvr_mse, _, results_SES_mvr = SES.model(X, y, ll=ll, at=True, NUM_TRIALS=5, cv=3, depths1=[2,3,4], depth2=2, martingale_indices=[0])

        # results = {'rbf': results_RBF, 'Matern': results_Matern, 'GA': results_GA, 'KES': results_KES, 'SES': results_SES, 'SES_mvr': results_SES_mvr}
        print({'SES': SES_mse, 'SES_mvr': SES_mvr_mse})
